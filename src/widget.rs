...
pub fn widget_knob(gui_renderer: &mut UiRenderer, display: &mut GlutinFacade, rect: Rect, knob: &KnobUiState) {
...
pub fn widget_button(gui_renderer: &mut UiRenderer, display: &mut GlutinFacade, rect: Rect, button: &ButtonUiState) {
	const GRAY: f32 = 0.1;
	gui_renderer.queue_rect(RenderRect {rect: rect, color: [GRAY, GRAY, GRAY, 1.], filled: button.pressed, line_width: 0.5});
...
pub fn widget_toggle_button(gui_renderer: &mut UiRenderer, display: &mut GlutinFacade, rect: Rect, button: &ToggleButtonUiState) {
	gui_renderer.queue_rect(RenderRect {rect: rect, color: COLOR_WHITE, filled: button.on, line_width: 0.2});
